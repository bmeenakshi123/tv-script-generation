...
def token_lookup():
    ...
    token_dict = {".":"||Period||", ",":"||Comma||",'"':"||Quotation_Mark||",";":"||Semicolon||",
                 "!":"||Exclamation_Mark||", "?":"||Question_Mark||", "(": "||Left_Parantheses||",")":"||Right_Parantheses||",
                 "--":"||Dash||", "\n":"||Return||"}
...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    predictions = tf.contrib.layers.fully_connected(outputs, num_outputs = vocab_size, activation_fn=None)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    for idx in range(n_batches):        
        ...
        for ii in range(batch_size):
            index = idx*seq_length + (ii*skip)
            batches[idx][0][ii] = int_text[index:index+seq_length]        
            batches[idx][1][ii] = int_text[index+1:index+1+seq_length]
    batches[idx][1][ii][seq_length-1]= batches[0][0][0][0]       
    return batches
